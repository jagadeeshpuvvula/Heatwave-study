...
#
dat<- read_csv("/work/jessebell/puvvula/h_def_2021nov.csv")
dat_n<- dat %>% select("date", "Region", "shw_08", "shw_15", "NWS_HW")

#long to wide
dat_w<- dat_n %>% pivot_wider(names_from = Region, values_from = c(shw_08,shw_15, NWS_HW))

#McNemar test (Matched/paired t-test)
#Comparing SHW_15 - coastal with shw_08 - piedmont
table(dat_w$shw_15_Coastal, dat_w$shw_08_Piedmont) #2 by 2 table
prop.table(table(dat_w$shw_15_Coastal, dat_w$shw_08_Piedmont),margin = 2)*100 #proportion table
mcnemar.test(dat_w$shw_15_Coastal, dat_w$shw_08_Piedmont)
...
#PIEDMONT : Comparing shw_08 with NWS_HW
table(dat_w$shw_08_Piedmont, dat_w$NWS_HW_Piedmont) #2 by 2 table
prop.table(table(dat_w$shw_08_Piedmont, dat_w$NWS_HW_Piedmont),margin = 2)*100 #proportion table
mcnemar.test(dat_w$shw_08_Piedmont, dat_w$NWS_HW_Piedmont)
```


```{r}
#figure 2
dat<- read_csv("/work/jessebell/puvvula/h_def_2021nov.csv") %>% select("date", "Region", "tmax", "tavg","shw_08", "shw_15", "NWS_HW","imp_rate")


#subset region
df<- dat %>% filter(Region=="Coastal")
df$hw<- factor(as.numeric(as.factor(!df$shw_08):as.factor(!df$NWS_HW)))
