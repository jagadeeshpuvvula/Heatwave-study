...
piedmont<-read_csv ("/work/jessebell/puvvula/coastal_octf.csv")
...
m1<- gam(imp_rate ~ s(temp_c,k=4, bs='cr')+wDay+month+year,
...
m2<- gam(imp_rate ~ s(temp_c,k=5, bs='cr')+wDay+month+year,
         family=Gamma(link = log),
         method = "GCV.Cp",
         data=piedmont)
...
tbl<-table(coastal_dat$Category, coastal_dat$hot_day)
chisq.test(tbl)
...
dat_test<-gather(pied_dat, scene, measure, 
...
dat_two %>% 
  group_by(Region) %>% 
  summarize(q1 = quantile(anomaly, 0.25),
            q2 = quantile(anomaly, 0.50),
...
coastal_dat<-filter(dat_three,region=="Coastal")
pied_dat<-filter(dat_three,region=="Piedmont")
